...
export const TRACKER_VARIABLE_GUIDE_PROMPT = [
  ...
  '- stage: 当前阶段。可能是月经阶段、妊娠阶段、假孕期、回归期、产兆前驱、第一/第二/第三产程、产后恢复、哺乳期、围绝经期晚期、停经、无经期、未激活。',
  ...
  '- fetuses[*].provider: 胚胎真正的归属方（代孕委托者等），自然受孕为 null。单一 provider 的孩子分娩后自动转交。要建立外源受精卵请用 bsImplantEmbryo，不要自行编造。',
  '- fetuses[*].providerSources: 可接收孩子的母源名单。多于一位时孩子默认登记在孕母名下，之后可手动转移给其中一位。',
  '- fetuses[*].tags: 系统标注的胎儿来历标签（如 surrogacy/identical），由系统推导或在事件发生当下写入，只读，不要自行增删。本轮出现过的标签会在下方另行说明。',
