...
export const TRACKER_VARIABLE_GUIDE_PROMPT = [
  ...
  '- profile 主要包含 base、pregnant、experience、sexuality、psychology、skills、talents、children、metabolism、descriptions、diary、notify，必要时也会附带部分 bio 字段。',
  ...
  '- sperms[*].value: 当前残留量，用于受精判定；每天自动衰减 10，无需手动清除。体内同一时间只有一名来源。',
  '- eggs: 当前可受精卵子数。',
  '- libido: 性欲。',
  ...
  '- stageProgressText / hormonePhase: 系统附带的阶段进度文本与激素阶段键（黄体期、卵泡期、月经期各分早晚段，黄体期晚段即 PMS 窗口）；只读，不要写入。',
  '- lactationDaysLeft: 哺乳期角色附带的断奶窗口剩余天数；有效哺乳（缓解乳意≥10）会重置计时。只读。',
  '',
  '[pregnant]',
  '- pregnant 只会在已有 fetuses、妊娠阶段、产兆前驱/产程、产后恢复发送；幕外角色发送时只保留少量 pregnant 摘要，并用 fetusesCount 表示胎儿数量。',
  '- pregnantDays: 这次妊娠的孕龄天数，等同产科从末次月经/等价周期起点计算的孕周天数。',
  ...
  '',
  '[sexuality]',
  '- sexuality 是角色稳定的性爱观与癖好画像（注册时设定）：intimacyNeed 性爱观、initiative 主被动、varietyTaste 花样偏好、contraceptionStyle 避孕偏好、note 癖好补充。',
  '- 它对所有工具都是只读资料，任何工具都不得修改；需要调整只能由用户在变量界面改。',
  '- 写 psychology 与 diary 时应与该画像一致：例如 intimacyNeed=love_only 的角色不会因为一次无爱的性而自然提升亲密类心理，contraceptionStyle=pill 的角色不会突然坚持要戴套。',
  '',
  '[psychology]',
  ...
  '',
  '[notify]',
  '- firstly: 主要阶段变化或必须优先处理的警示，例如进入孕早期/哺乳期/初潮/停经的阶段切换、真实产程中的难产手术产建议。',
...
export function buildTrackerSystemPrompt(basePrompt = '', descriptionGuides = null, payload = null) {
  ...
  const priorityNames = Array.isArray(payload?.priority_character_names)
    ...
    : [];
  const hormonePhasesPrompt = describeHormonePhases(payload || {});
  if (hormonePhasesPrompt) parts.push(hormonePhasesPrompt);
